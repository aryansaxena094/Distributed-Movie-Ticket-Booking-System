...
public class PublishOUT {
    ...
    public static void StartService(){
        endpoint1 = Endpoint.publish("http://172.20.10.9:8082/movieOUT", new OUTImpl());
    ...
    public static void main(String[] args) throws SocketException, IOException, InterruptedException, ClassNotFoundException {
        ...
        DatagramSocket ds = new DatagramSocket(3036);
        ...
        while (true) {
            ...
            if (rec.substring(0, 4).equals("list")) {
                ...
                } else if (rec.substring(4, 7).equals("VER")) {
                ...
                DatagramPacket packet2 = new DatagramPacket(buf, buf.length, ip, 3036);
                ...
                DatagramPacket initialPacket = new DatagramPacket(buf, buf.length, address, 3036);
                socket.send(initialPacket);
                Thread.sleep(1000);
                DatagramPacket packet = new DatagramPacket(byteData, byteData.length, address, 6003);
                socket.send(packet);
                socket.close();
            } else if (rec.equals("receiveHM")) {
                HashMap<String, HashMap<String, Double>> data = new HashMap<String, HashMap<String, Double>>();
                DatagramSocket dsHM = new DatagramSocket(6003); 
