...
public class VERImpl implements IVER{
    ...
    public String listMovieShowsAvailability(String movieName) {
        ...
        try {       
            ...
            String inp = "listVER" + movieName;
            buf = inp.getBytes();
            DatagramPacket DpSend = new DatagramPacket(buf, buf.length, ip, 3036);  
            LogWritterGeneral("Not in request", "listMovieShowsAvailability-UDP Request", inp, "Success", "request sent");
            ds.send(DpSend);
            HashMap<String, Double> data = new HashMap<String, Double>();
            DatagramSocket ds1 = new DatagramSocket(3002); 
    ...
    public String getBookingSchedule(String customerID) {
        ...
        try {       
            ...
            ds.send(DpSend);
            HashMap<String, Double> data = new HashMap<String, Double>();
            DatagramSocket ds1 = new DatagramSocket(3005); 
            byte[] receive = new byte[65535];
            DatagramPacket DpReceive = null;
            while (data.isEmpty()) {
                DpReceive = new DatagramPacket(receive, receive.length);
