...
public class PublishATW {
    ...
    public static void listMovieShowsForOtherServer(String movieName, String port) throws IOException {
        ...
        if (port.equals("VER")) {
            DatagramPacket packet = new DatagramPacket(byteData, byteData.length, address, 3002);
            socket.send(packet);
            socket.close();
    ...
    public static void getBookingScheduleforOthers(String customerID, String port) throws IOException {
        ...
        InetAddress address = InetAddress.getByName("localhost");
        if (port.equals("VER")) {
            DatagramPacket packet = new DatagramPacket(byteData, byteData.length, address, 3005);
            socket.send(packet);
            socket.close();
        } else if (port.equals("OUT")) {
    ...
    public static void getMovieAvlForOthers(String input, String port) throws SocketException, UnknownHostException, IOException {
        ...
        buf = avl.getBytes();
        if (port.equals("VER")) {
            DatagramPacket packet = new DatagramPacket(buf, buf.length, ip, 3005);
            ds.send(packet);
            ds.close();
        } else if (port.equals("OUT")) {
    ...
    public static void StartService() {
        endpoint1 = Endpoint.publish("http://172.20.10.9:8080/movieATW", new ATWImpl());
    ...
    public static void main(String[] args) throws SocketException, IOException, ClassNotFoundException, InterruptedException {
        ...
        DatagramSocket ds = new DatagramSocket(3034);
        ...
        while (true) {
            ...
            if (rec.substring(0, 4).equals("list")) {
                if (rec.substring(4, 7).equals("VER")) {
                    inp = rec.replace("listVER", "");
                    listMovieShowsForOtherServer(inp.trim(), "VER");
                ...
                if (rec.substring(4, 7).equals("VER")) {
                    inp = rec.replace("bkngVER", "");
                    getBookingScheduleforOthers(inp.trim(), "VER");
                } else if (rec.substring(4, 7).equals("OUT")) {
                    inp = rec.replace("bkngOUT", "");
                    getBookingScheduleforOthers(inp.trim(), "OUT");
                }
            } else if (rec.substring(0, 6).equals("chkavl")) {
                if (rec.substring(6, 9).equals("VER")) {
                    inp = rec.replace("chkavlVER", "");
                    getMovieAvlForOthers(inp.trim(), "VER");
                } else if (rec.substring(6, 9).equals("OUT")) {
                ...
                DatagramPacket packet2 = new DatagramPacket(buf, buf.length, ip, 3034);
                ...
                DatagramPacket initialPacket = new DatagramPacket(buf, buf.length, address, 3034);
                socket.send(initialPacket);
                Thread.sleep(1000);
                DatagramPacket packet = new DatagramPacket(byteData, byteData.length, address, 6001);
                socket.send(packet);
                socket.close();
            } else if (rec.equals("receiveHM")) {
                HashMap<String, HashMap<String, Double>> data = new HashMap<String, HashMap<String, Double>>();
                DatagramSocket dsHM = new DatagramSocket(6001); 
